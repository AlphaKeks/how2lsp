-- Install packer (our plugin manager) automatically if it's not already
-- installed. It will clone the repo as soon as you open neovim. Then re-open
-- neovim and you should have access to various `:Packer` commands.
-- The first one of those that you should run is `:PackerSync` to install all
-- your plugins.
...
require("packer").startup(function(use)
	...
	-- preset LSP configurations + easier to use API
	-- read this for all available language servers:
	--   https://github.com/neovim/nvim-lspconfig/blob/master/doc/server_configurations.md
	use("neovim/nvim-lspconfig")

	-- completion enginge that can pickup from language servers and other sources
	use({
		"hrsh7th/nvim-cmp",
		requires = {
			-- nvim-cmp requires a snippet enginge to work, even if you're not gonna use snippets.
			"L3MON4D3/LuaSnip",

			-- These are called "sources" -- they are feeding
			-- autocompletion results into nvim-cmp, which will show them
			-- on your screen.
...
-- `pcall` returns a boolean + the return value of the function you pass to it.
-- The boolean determines whether the function call succeeded or not without
-- crashing your program if it fails.
...
if treesitter_installed then
	treesitter.setup({
		...
		-- servers are installed and up to date.
		ensure_installed = {
			"help",
			"vim",
			"rust",
		...
		-- cause problems in some languages (e.g. python) so you might
		-- need to disable it for certain filetypes.
...
if lspconfig_installed then
	...
	local function lsp_keymaps(buf_number)
		...
		-- This will jump your cursor to the next available diagnostic
		-- and display it.
		vim.keymap.set("n", "gL", vim.diagnostic.goto_next)
	end

	-- The diagnostic API is not exclusive to LSP but heavily used by it.
	-- If you want it to look fancy, you should configure it :)
	-- :h vim.diagnostic.config
	vim.diagnostic.config({
		-- Will display errors on the line they occur in.
		virtual_text = {
			-- Will show which language server/source the
			-- diagnostic message is coming from.
			source = true,
			...
			-- anything. You can still read them with the `gl`
			-- keymap from above, they just won't show inline.
		...
		float = {
			-- This will enable you to press `gl` again and put
			-- your cursor inside the window so you can move around
			-- and copy stuff. You can go back with `<esc>`,
			-- `<C-o>` or `q`.
			focusable = true,
	...
	-- This section will make more sense once we move onto `nvim-cmp` but
	-- just know that this is necessary to get stuff like auto-imports when
	...
	-- install and configure them; but it's the same pattern every time.
end
...
if cmp_installed and luasnip_installed then
	cmp.setup {
		...
		sources = {
			...
			-- We only want 1 buffer word to be shown at a time,
			-- feel free to change this of course.
			-- `keyword_length` will determine how many characters
			-- you need to type before a word gets suggested.
		...
		formatting = {
			...
			format = function(_, item)
				...
				-- If you have a font with icon support
				-- installed you can use these, they will show
				-- up in your completion menu based on the kind
				-- of item you get suggested. If you don't have
				-- such a font or just simply prefer words,
				-- just remove this line and it will use words
				-- instead.
				item.kind = ({
